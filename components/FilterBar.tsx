  ...
  return (
    <div className="card p-4 flex flex-col gap-4">
      <div className="relative">
        <Search
          size={14}
          className="absolute left-3 top-1/2 -translate-y-1/2 text-fog pointer-events-none"
        />
        <input
          type="text"
        ...
        {filters.search && (
          <button
            onClick={clearSearch}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-fog hover:text-rose transition-colors">
            <X size={14} />
          </button>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-3">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <PillGroup
            options={viewOptions}
            active={filters.minViews}
            onSelect={(value) => setFilters({ minViews: value })}
          />
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <PillGroup
            ...
            onSelect={(value) => setFilters({ contentType: value })}
          />
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <PillGroup
            ...
            onSelect={(value) => setFilters({ sortKey: value })}
          />
        </div>
      </div>
    </div>
